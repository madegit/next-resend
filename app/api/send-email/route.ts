    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      for (const [key, value] of formData.entries()) {
        if (key.startsWith("attachment") && value instanceof File) {
          ...
          attachmentCount++;
          console.log(
            `- Attachment ${attachmentCount}: ${value.name} (${value.size} bytes, ${value.type})`,
          );
        }
      }
      ...
      if (data.attachments && Array.isArray(data.attachments)) {
        attachments = data.attachments.map((att: Base64Attachment) => ({
          filename: att.filename,
          content: Buffer.from(att.content, "base64"),
        }));
        console.log("- Base64 attachments:", data.attachments.length);
        data.attachments.forEach((att: Base64Attachment, index: number) => {
          console.log(
            `  - Attachment ${index + 1}: ${att.filename} (base64 length: ${att.content?.length || 0})`,
          );
        });
      } else {
